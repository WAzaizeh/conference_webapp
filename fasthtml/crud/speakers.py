...
from .common import get_db_speaker, _get_db_speaker, get_db_event
...
def create_db_speaker(speaker: SpeakerCreate, db: Session) -> SpeakerOut:
    db_speaker = DBSpeaker(**speaker.model_dump(exclude={'event_ids'}))
...
def update_db_speaker(speaker_id: int, speaker: SpeakerUpdate, db: Session) -> SpeakerOut:
    ...
    if not db_speaker:
        return None
    update_data = speaker.model_dump(exclude_unset=True)
